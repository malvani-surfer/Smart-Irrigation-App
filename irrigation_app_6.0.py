...
# --- Function to add Background Image & Custom CSS ---
def add_custom_style():
    st.markdown(
         ...
         /* 2. White Overlay for Main Content Area ONLY */
         .stApp::before {{
             content: "";
             position: absolute;
             top: 0;
         ...
         /* 3. Main Content Text (Dark & Bold) */
         .main h1, .main h2, .main h3, .main h4, .main h5, .main h6, .main p, .main span, .main label, .main div {{
...
if st.button("🚀 Calculate Irrigation Schedule"):
    ...
    if run_time > 60:
        st.warning(f"⚠️ **High Water Demand!** Run pump for **{int(run_time)} minutes** ({run_time/60:.1f} hours).")
    else:
        st.success(f"✅ **Optimal Condition.** Run pump for **{int(run_time)} minutes**.")
