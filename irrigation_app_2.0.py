...
Original file is located at
    https://colab.research.google.com/drive/17ZVy-OQM6VKExvrHeyIHeMb8vlq-HUG7
"""
...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://images.unsplash.com/photo-1625246333195-f8196812c850?q=80&w=2070&auto=format&fit=crop");
         ...
         .stApp::before {{
             ...
             width: 100%;
             height: 100%;
             background-color: rgba(255, 255, 255, 0.80); /* White overlay with 80% opacity */
             z-index: -1;
         }}
         </style>
         """,
...
if st.button("Calculate Irrigation Schedule"):
    ...
    # Advice Box
    if run_time > 60:
        st.warning(f"⚠️ High Water Demand! Run pump for {int(run_time)} minutes. Check soil moisture manually.")
    else:
        st.success(f"✅ Normal Condition. Run pump for {int(run_time)} minutes.")
