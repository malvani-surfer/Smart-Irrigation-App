...
if st.button("🚀 Calculate Irrigation Schedule"):
    ...
    # Graph 1: Water Requirement Breakdown (Pie Chart)
    with chart_col1:
        st.markdown("**1. Water Use Efficiency**")
        st.caption("Estimated water loss vs. effective crop intake.")

        # Simple breakdown logic for demo
        # Assuming ~70% is efficient crop use, 30% is potential loss (evaporation/leaching)
        labels = ['Effective Crop Water', 'System Losses (Est.)']
        sizes = [water_vol * 0.7, water_vol * 0.3]
        colors = ['#4CAF50', '#FFC107'] # Green for crop, Amber for loss
    ...
    # Graph 2: Monthly Irrigation Trend (Bar Chart)
    with chart_col2:
        st.markdown("**2. Seasonal Planning Guide**")
        st.caption("Projected average daily pump runtime per month (Historical Trends).")

        # Simulated historical average runtimes for Kamshet region
        months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
        # Summer peak in Apr/May
        avg_runtimes = [45, 50, 65, 80, 90, 60]

        df_trend = pd.DataFrame({
            "Month": months,
            "Avg Runtime (Mins)": avg_runtimes
        })

        st.bar_chart(df_trend.set_index("Month"), color="#2196F3")
